//VARIÁVEIS
var palavras = ["BANANA","ABACAXI","ABACATE","AMORA","ACEROLA","CEREJA","FIGO","JABUTICABA","LARANJA","MANGA","PITANGA","UVA"]
var letras = [];
...
var adicionaPalavraBtn = document.getElementById("adiciona-palavra");
...
function verificaPalavraDoInput() {
    valorPalavra = (inputAddPalavra.value).toUpperCase();
        if ((valorPalavra.length < 4) || (valorPalavra.length > 8)){
            alert("Digite o nome de uma FRUTA válida e com até 8 letras!");
            inputAddPalavra.value = "";
            inputAddPalavra.focus();

        }
        else if (palavras.includes(valorPalavra)) {
            alert("Essa fruta já está na lista, tente outra!")
            inputAddPalavra.value = "";
            inputAddPalavra.focus();
        }
        else{
